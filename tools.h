...
void obtenerDatos(istream &file,int nlines,int n,int mode,item* item_list){
    ...
    for(int i=0;i<n;i++){
        ...
        case INT_FLOAT:
            int e0; float r0;
            file >> e0 >> r0;
            item_list[i].setValues(0,0,0,r0,0,e0,0,0);
            break;
...
void writeResults(mesh m,vector<Vector> Ts,char *filename){
    ...
    Vector T;
	cout<<"estoy aqui en writeResults"<<endl;
    ...
    file << "GiD Post Results File 1.0\n";
		cout<<"estoy a punto de entrar al for 1 "<<endl;
    ...
    for(int step=1;step<=Ts.size();step++){
		...
		cout<<"Acabando el primer for"<<endl;
        for(int i=0;i<nn;i++){
		cout<<"Entrando al segundo for "<<endl;
            int d_index = getIndex(i+1+2*nn,nd,dirich_indices);
			cout<<"indices listos "<<d_index<<endl;
            if(d_index != -1)
                file << i+1 << " " << dirich[d_index].getValue() << "\n";
            else{
                int T_index = getIndex(i+1+2*nn,4*nn-nd,non_dirich_indices);
				cout<<"tindex "<<T_index<<endl;
                file << i+1 << " " << T.at(T_index) << "\n";
            }
			cout<<"Iteracion for 2 "<<i<<endl;
        }
